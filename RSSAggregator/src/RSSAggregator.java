...
public final class RSSAggregator {
    ...
    private static void outputHeader(XMLTree channel, SimpleWriter out) {
        ...
        out.println("<a href=\""
                ...
                + "</a>");
        if (channel.child(getChildElement(channel, "description")).isTag()) {
            out.println("<p>"
                    + channel.child(getChildElement(channel, "description"))
                    + "</p>");
        } else {
            out.println("<p>" + "desciption not available" + "</p>");
        }
    ...
    private static int getChildElement(XMLTree xml, String tag) {
        ...
        for (int i = 0; i < xml.numberOfChildren(); i++) {
            if (xml.child(i).label().equals(tag) && xml.child(i).isTag()) {
                index = i;
                //setting i to the number of children will short curcuit
    ...
    private static void processFeed(String url, String file, SimpleWriter out) {
        ...
        if (xml.label().equals("rss") && xml.hasAttribute("version")
            ...
            out.println(url
                    + "points to an invalid RSS document. Moving on to next feed...");
        }
    }
    ...
    public static void main(String[] args) {
        ...
        consoleOut
                .println("Please enter the name of an xml file with multiple RSS feeds(must be version 2.0): ");
        SimpleReader in = new SimpleReader1L();
        String indexPage = in.nextLine();
        XMLTree xml = new XMLTree1(indexPage);
        consoleOut
                .println("Now enter a name for HTML page that will contain the links to the given RSS feeds: ");
        String mainHtmlPage = in.nextLine() + ".html";
        SimpleWriter toIndex = new SimpleWriter1L(mainHtmlPage);

        toIndex.print("<html>");
        toIndex.println("<head>");
        toIndex.println("<title>" + xml.attributeValue("title") + "</title>");
        toIndex.println("<body>");
        toIndex.println("<h2>" + xml.attributeValue("title") + "</h2>");
        toIndex.println("<h2>");
        toIndex.println("<ul>");
        for (int j = 0; j < xml.numberOfChildren(); j++) {
            toIndex.println("<li><a href=\""
                    + xml.child(j).attributeValue("file") + "\">"
                    + xml.child(j).attributeValue("name") + "</a></li>");
        ...
        for (int i = 0; i < xml.numberOfChildren(); i++) {
            ...
            processFeed(url, name, consoleOut);

        }

        //http://www.cse.ohio-state.edu/software/2221/web-sw1/assignments/projects/rss-aggregator/feeds.xml
        toIndex.close();
        consoleOut.close();
        in.close();
    }
